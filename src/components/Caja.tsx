...
export default function Caja({ shift, onCloseShift }: CajaProps) {
  ...
  const expectedCash = Number(shift?.opening_cash || 0) + balance;
      ...
      <div className="bg-gradient-to-r from-slate-700 to-slate-900 rounded-xl p-6 text-white shadow-xl">
        <div className="flex items-center justify-between">
          <div className="space-y-2">
            ...
            <p className="text-lg">
              <span className="text-slate-300">Efectivo Inicial:</span> <strong>${Number(shift.opening_cash).toFixed(2)}</strong>
            </p>
          </div>
          <button
        ...
        <table className="w-full">
          <thead className="bg-slate-100">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase">Monto</th>
              <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase">Método</th>
              <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase">Descripción</th>
            </tr>
          </thead>
          <tbody>
            {transactions.map((t) => (
              <tr key={t.id} className="border-t border-slate-200 hover:bg-slate-50">
                ...
                <td className="px-6 py-4 text-sm font-bold text-slate-800">${Number(t.amount).toFixed(2)}</td>
            ...
            <div className="p-6 space-y-4">
              <div className="bg-slate-50 rounded-xl p-4 space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-900">{new Date(shift.start_date).toLocaleString('es-AR')}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-semibold text-slate-700">Efectivo Inicial:</span>
                  <span className="text-slate-900">${Number(shift.opening_cash).toFixed(2)}</span>
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-lg font-bold text-blue-600">${expectedCash.toFixed(2)}</span>
                </div>
              </div>
              ...
              {closingCash && (
                <div className={`p-4 rounded-xl ${
                  ...
                  <p className="font-semibold text-sm">
                    {Math.abs(parseFloat(closingCash) - expectedCash) < 0.01 ? (
                      <span className="text-emerald-700">✓ La caja cuadra perfectamente</span>
                    ) : parseFloat(closingCash) > expectedCash ? (
                      <span className="text-blue-700">Hay ${(parseFloat(closingCash) - expectedCash).toFixed(2)} de más</span>
                    ) : (
                      <span className="text-amber-700">Faltan ${(expectedCash - parseFloat(closingCash)).toFixed(2)}</span>
                    )}
                  </p>
                </div>
              )}
